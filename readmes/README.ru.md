...
4-ядерном раннере это означает, что **SUT — это 2 cpu** (`cpuset 0-1`) с `cpus=2` и
`mem_limit=4g`, то есть Docker-ограниченный сервер 2 CPU / 4 GB; `wrk` работает на `2-3`;
на 8-ядерном хосте SUT получает 4 cpu (`0-3`), а `wrk` — `4-7`. Компромисс в том, что
SUT получает лишь **половину машины** — поэтому на стандартном раннере отчёты для **2-cpu /
4 GB сервера**, записанного в лимитах манифеста (`cpus=2`, `mem=4g`). Поскольку общие раннеры CI к тому же
...
По умолчанию `benchmark.sh` тестирует примерно `2 * SUT_CPUS` воркеров, а затем вдвое больше.
На стандартном 4-vCPU раннере SUT получает 2 CPUs, поэтому стандартный sweep воркеров — `4 8`.
Если 8 workers дают меньший throughput или худший p99, чем 4, это валидный результат: обычно это
значит, что дополнительные PHP workers добавляют contention планировщика, давление на cache или
DB/socket contention, не добавляя реальной CPU-ёмкости.
...
- **2-cpu / 4 GB SUT на 4-ядерном раннере.** Чтобы держать генератор изолированным, хост делится
  пополам — поэтому на стандартном раннере каждый тестируемый app Docker container — это
  **2 CPU / 4 GB** сервер (остальные 2 ядра гонят `wrk`). Это помечено в манифесте
  (`cpus=2`, `mem=4g`). Для 4-cpu SUT *с* изолированным
  генератором нужен 8-ядерный хост (разделение тогда даёт SUT 4 ядра, а `wrk` — другие 4).
- **Больше workers не всегда быстрее.** Падение с 4 до 8 workers следует читать как найденную
  локальную точку насыщения этой машины, особенно для CPU-bound workloads или стандартного
  разделения SUT на 2 CPU.
...
[Terry L.](https://terryl.in). Terry L. также является разработчиком Airygen, бесплатного и
мощного [WordPress SEO Plugin](https://www.airygen.com/ru) для команд, которым нужны
структурированные workflows контента и инструменты публикации, ориентированные на поиск.

## Открытое обсуждение

Serverless container-сервисы, например Google Cloud Run, могут вести себя иначе, чем фиксированный
2-CPU container, использованный в этом benchmark. Такие сервисы обычно тарифицируются по выделенному
compute и могут работать на хостах с большим числом базовых CPU cores. Поэтому большее число workers
теоретически может потреблять доступный compute до достижения настроенного лимита сервиса. В такой
среде правило `workers = CPU x 2` не обязательно является правильным значением по умолчанию; worker
count следует настраивать с учётом фактического CPU allocation платформы, concurrency model, модели
тарификации и latency target.
