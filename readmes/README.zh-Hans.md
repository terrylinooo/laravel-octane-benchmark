...
**SUT 占 2 cpus**（`cpuset 0-1`），并设置 `cpus=2`、`mem_limit=4g`，也就是一个被 Docker
资源限制住的 2 CPU / 4 GB server；`wrk` 跑在 `2-3` 上；在 8 核主机上 SUT 拿到 4 cpus
（`0-3`），`wrk` 拿到 `4-7`。代价是 SUT 只拿到**半台机器**——所以在默认 runner 上，
报告针对的是一个 **2-cpu / 4 GB 服务器**，记录在清单的上限值中（`cpus=2`、`mem=4g`）。由于共享 CI
...
`benchmark.sh` 默认会测试大约 `2 * SUT_CPUS` 的 worker 数，然后再测试它的两倍。默认 4-vCPU
runner 会让被测 server 拿 2 CPUs，所以默认 worker sweep 是 `4 8`。如果 8 workers 的吞吐量
低于 4，或 p99 更差，这是有效结果：通常代表额外 PHP workers 带来 scheduler contention、
cache pressure，或 DB/socket contention，但没有增加真正可用的 CPU 容量。
...
- **4 核 runner 上的 2-cpu / 4 GB SUT。** 为保持压测工具隔离，主机被一分为二——所以在默认
  runner 上每个被测 app Docker container 都是 **2 CPU / 4 GB** server（另外 2 个核心驱动 `wrk`）。
  这在清单中有标注（`cpus=2`、`mem=4g`）。若要在**带**隔离压测工具的同时拥有 4-cpu SUT，你需要一台 8 核主机
  （届时切分会给 SUT 4 个核心，`wrk` 拿到另外 4 个）。
- **更多 workers 不一定更快。** 4 workers 到 8 workers 反而下降时，应解读成 benchmark 找到
  这台机器上的局部饱和点，尤其是在 CPU-bound workload 或默认 2-CPU SUT 切分下。
...
这份 Laravel Octane benchmark 以 MIT License 发布，由 [Terry L.](https://terryl.in)
维护。Terry L. 同时也是 Airygen 的开发者，它是一套免费且强大的
[WordPress SEO Plugin](https://www.airygen.com/zh-Hans)，适合需要结构化内容流程与搜索导向发布工具的团队。

## 待讨论

Serverless container 服务，例如 Google Cloud Run，和本 benchmark 使用的固定 2-CPU container
不一定是同一种行为。这类服务通常按配置算力计费，而且底层 host 可能有很多 CPU cores；worker
数量较多时，理论上可以持续消费可用算力，直到撞到服务设置的算力上限。在这种环境下，
`workers = CPU x 2` 这个经验公式不一定适合作为默认值；worker count 应该根据平台实际 CPU
配置、concurrency model、计费方式与 latency target 重新调整。
