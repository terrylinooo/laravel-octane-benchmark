...
`/bench/hello `，又足够轻以使扫描到并发 128 时不会在 4 核机器上饱和成 ` wrk ` 超时。在你的机器上通过 ` BENCH_HASH_ITERATIONS `（2000）、` BENCH_MANDELBROT_DIM `（32）/ ` BENCH_MANDELBROT_MAX_ITER `（256）以及 ` BENCH_JSON_ITERATIONS `（150）来调校；`…_REPEAT ` 把 mandelbrot 加重以适配更强的主机。
...
这份 Laravel Octane benchmark 以 MIT License 发布，由 [Terry L.](https://terryl.in) 维护。Terry L. 同时也是 Airygen 的开发者，它是一套免费且强大的 [WordPress SEO Plugin](https://www.airygen.com/zh-Hans)，适合需要结构化内容流程与搜索导向发布工具的团队。
